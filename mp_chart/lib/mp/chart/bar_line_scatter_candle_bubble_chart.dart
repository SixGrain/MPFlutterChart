...
import 'package:mp_chart/gesture/details.dart';
import 'package:mp_chart/gesture/direction.dart';
...
class BarLineScatterCandleBubbleState<T extends BarLineScatterCandleBubbleChart>
  ...
  void onSingleTapUp(TapUpDetails details) {
    if (widget.controller.painter?.highLightPerTapEnabled == true) {
  ...
  void onDoubleTapUp(TapUpDetails details) {
    widget.controller.stopDeceleration();
    if (widget.controller.painter?.doubleTapToZoomEnabled == true &&
  ...
  void onMoveUpdate(OpsMoveUpdateDetails details) {
    ...
    if (widget.controller.painter?.highlightPerDragEnabled == true) {
    ...
    if (widget.controller.painter?.dragYEnabled == true &&
      ...
      if (widget.controller.painter?.dragXEnabled == true) {
        if (_inverted()) {
          /// if there is an inverted horizontalbarchart
          if (widget is HorizontalBarChart) {
            dx = -dx;
        ...
        needStateIfNotDispose = true;
      } else if (widget.controller.painter?.dragYEnabled == true) {
        if (_inverted()) {
          /// if there is an inverted horizontalbarchart
          if (widget is HorizontalBarChart) {
            dx = -dx;
  ...
  void updatePainter() {
    if (widget.controller.painter?.getData() != null &&
        ...
        (widget.controller.painter?.getData()?.dataSets.length ?? 0) > 0)
